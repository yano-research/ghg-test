...
export default function IndustryListPage() {
  ...
  useEffect(() => {
    ...
    async function fetchCompanies() {
        const { data, error } = await supabase
          .from("emissions3")
          .select("name, company_number, scope, total_emission")
          .eq("source", "gov")
        ...
        const grouped = data.reduce((acc, row) => {
          ...
          if (!acc[key]) {
            acc[key] = { name: row.name, total: 0 }
          }
          if (row.scope === 1 || row.scope === 2) {
            acc[key].total += row.total_emission || 0
          }
          return acc
        }, {})
    ...
    <div className="p-10 space-y-6">
      ...
      {selectedIndustry && (
        <div className="bg-white rounded-xl shadow p-6 mt-4">
          <h2 className="text-md font-semibold mb-4">{selectedIndustry} 業種の企業一覧</h2>
              ...
              <tbody>
                {companyData.map((row, i) => (
                  <tr key={i} className="border-t">
                    ...
                    <td className="py-1 text-right">{row.total.toLocaleString()} t-CO₂</td>
                  </tr>
                ))}
              </tbody>
            </table>
